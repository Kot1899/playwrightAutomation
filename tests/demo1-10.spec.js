...
test('check app 1', async ({ page }) => {
  ...
  await expect(await page.locator("(//div[@class='text-white text-[32px] leading-[42px] font-semibold whitespace-normal text-center sm:text-left line-clamp-4'])[1]")).toContainText(appName, ( appName, { timeout: 7000 }));  
  await expect(await page.locator("(//div[@class='text-[#A7A7A7] text-sm whitespace-normal text-center sm:text-left line-clamp-4'])[1] ")).toContainText(appDescription); 
  await expect(await page.locator("(//div[@class='text-white text-sm font-medium my-10'])[1]")).toContainText(appInfo);
