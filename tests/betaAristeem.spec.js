...
test('check app 1', async ({ page }) => {
  ...
  await expect(await page.locator("(//div[@class='text-white text-[32px] leading-[42px] font-semibold whitespace-normal text-center sm:text-left line-clamp-4'])[1]")).toContainText(appName);     
  ...
  await expect(icon).toBeVisible();
   ...
   await expect(await page.getByAltText('image_0')).toBeVisible();
   await expect(await page.getByAltText('image_1')).toBeVisible();
   await expect(await page.getByAltText('image_2')).toBeVisible();
   await expect(await page.getByAltText('image_3')).toBeVisible();
});
...
test('check app 5', async ({ page }) => {
  ...
  await expect(await page.locator("(//div[@class='text-white text-[32px] leading-[42px] font-semibold whitespace-normal text-center sm:text-left line-clamp-4'])[1]")).toContainText(appName); 
  await expect(await page.locator("(//div[@class='text-[#A7A7A7] text-sm whitespace-normal text-center sm:text-left line-clamp-4'])[1] ")).toContainText("Professional CAD software for 2D and 3D design and drafting."); 
  await expect(await page.locator("(//div[@class='text-white text-sm font-medium my-10'])[1]")).toContainText("AutoCAD 2025 is a leading computer-aided design (CAD) software used by architects, engineers, and construction professionals to create precise 2D and 3D drawings. The 2025 version offers enhanced performance, new automation features, and improved collaboration tools. It supports various specialized toolsets and allows for customization through APIs.");
  await expect(await page.locator("(//div[@id='licenseType'])[1]")).toContainText("paid"); 
...
test('check app 6', async ({ page }) => {
  ...
  await expect(await page.locator("(//div[@class='text-white text-[32px] leading-[42px] font-semibold whitespace-normal text-center sm:text-left line-clamp-4'])[1]")).toContainText(appName); 
  await expect(await page.locator("(//div[@class='text-[#A7A7A7] text-sm whitespace-normal text-center sm:text-left line-clamp-4'])[1] ")).toContainText("Professional video editing, color correction, and audio post-production software."); 
  await expect(await page.locator("(//div[@class='text-white text-sm font-medium my-10'])[1]")).toContainText("DaVinci Resolve combines professional 8K editing, color correction, visual effects, and audio post-production in one software tool. It offers a non-linear editing system, advanced color grading, and Fairlight audio editing tools. It is widely used in film and television production for its powerful color correction capabilities.");
  await expect(await page.locator("(//div[@id='licenseType'])[1]")).toContainText("free"); 
...
test('check app 9', async ({ page }) => {
  ...
  await expect(await page.locator("(//div[@class='text-white text-[32px] leading-[42px] font-semibold whitespace-normal text-center sm:text-left line-clamp-4'])[1]")).toContainText(appName); 
  await expect(await page.locator("(//div[@class='text-[#A7A7A7] text-sm whitespace-normal text-center sm:text-left line-clamp-4'])[1] ")).toContainText("Professional video editing software for film and television."); 
  await expect(await page.locator("(//div[@class='text-white text-sm font-medium my-10'])[1]")).toContainText("Adobe Premiere Pro 2024 is a leading video editing application used by professionals to create movies, TV shows, and online videos. It offers advanced editing tools, support for high-resolution video formats, and seamless integration with other Adobe products. The 2024 version includes new features like enhanced color grading, improved performance, and collaborative editing tools. It supports a wide range of video formats and is suitable for both beginners and professionals.");
  await expect(await page.locator("(//div[@id='licenseType'])[1]")).toContainText("paid"); 
  await expect(await page.locator("(//div[@id='publisher'])[1]")).toContainText("Adobe"); 
  await expect(await page.locator("(//div[@id='developer'])[1]")).toContainText("Adobe"); 
  await expect(await page.locator("(//div[@id='category'])[1]")).toContainText("Video & Motion");
...
test('check app 10', async ({ page }) => {
  ...
  await expect(await page.locator("(//div[@class='text-white text-[32px] leading-[42px] font-semibold whitespace-normal text-center sm:text-left line-clamp-4'])[1]")).toContainText(appName); 
  await expect(await page.locator("(//div[@class='text-[#A7A7A7] text-sm whitespace-normal text-center sm:text-left line-clamp-4'])[1] ")).toContainText("Professional 3D animation, modeling, simulation, and rendering software."); 
  await expect(await page.locator("(//div[@class='text-white text-sm font-medium my-10'])[1]")).toContainText("Autodesk Maya 2025 is a powerful tool for creating expansive worlds, complex characters, and dazzling effects. Used in film, television, and game development, Maya offers tools for 3D modeling, animation, visual effects, and rendering. The 2025 version provides enhanced performance, new animation tools, and better integration with other Autodesk products.");
  await expect(await page.locator("(//div[@id='licenseType'])[1]")).toContainText("paid"); 
  await expect(await page.locator("(//div[@id='publisher'])[1]")).toContainText("Autodesk"); 
  await expect(await page.locator("(//div[@id='developer'])[1]")).toContainText("Autodesk"); 
  await expect(await page.locator("(//div[@id='category'])[1]")).toContainText("3D");
...
/*
test('check app 63', async ({ page }) => {
  await page.goto('https://demo.aristeem.com/programs/63');
  
  await expect(await page.locator("(//div[@class='text-white text-[32px] leading-[42px] font-semibold whitespace-normal text-center sm:text-left line-clamp-4'])[1]")).toContainText('Notch Builder'); 
  await expect(await page.locator("(//div[@class='text-[#A7A7A7] text-sm whitespace-normal text-center sm:text-left line-clamp-4'])[1] ")).toContainText("Real-time graphics and VFX creation tool"); 
  await expect(await page.locator("(//div[@class='text-white text-sm font-medium my-10'])[1]")).toContainText("Notch Builder is a software for creating interactive and generative content, motion graphics, and VFX in real-time. It offers a node-based workflow for designing visuals for live events, installations, and broadcast. The software integrates with media servers and supports real-time rendering and compositing. Notch Builder allows for complex visual effects without programming. It's widely used in the entertainment industry.");
  await expect(await page.locator("(//div[@id='licenseType'])[1]")).toContainText("paid"); 
  await expect(await page.locator("(//div[@id='publisher'])[1]")).toContainText("Notch"); 
  await expect(await page.locator("(//div[@id='developer'])[1]")).toContainText("10bit FX Ltd."); 
  await expect(await page.locator("(//div[@id='category'])[1]")).toContainText("Graphics");
...
/*
  const headerIcon = page.locator('img[src*="Adobe+Illustrator+2024/icon.webp"]');
  await headerIcon.waitFor({ timeout: 20000 }); 
  const headerIconSrc = await headerIcon.getAttribute('src');
  expect(headerIconSrc).toMatch("https://cdn.aristeem.com/images/Adobe+Illustrator+2024/icon.webp"); 
